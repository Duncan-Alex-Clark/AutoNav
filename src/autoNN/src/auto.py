...
def train(model, train_loader, criterion, optimizer, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        print("Saving model...")
        # torch.save(model.module.state_dict(), f"{modelPath}/last.pt")
        torch.save(model.state_dict(), f"{modelPath}/last.pt")
        print("Latest model saved!")
        if average_loss < prevLoss:
            # torch.save(model.module.state_dict(), f"{modelPath}/best.pt")
...
def main():
    ...
    # dataPath = f"~/IGVCDev/src/autoNN/data/training"
    ...
    train(model, train_loader, criterion, optimizer, 2000)
